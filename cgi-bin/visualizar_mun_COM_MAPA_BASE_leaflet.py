...
MunID = int(form['MunID'].value)

conn = psycopg2.connect("dbname = 'sig' port = '5432'  user= 'user' password = 'user' host='localhost'")
cursor = conn.cursor()
cursor.execute("""CREATE OR REPLACE VIEW escolhido AS SELECT * FROM bairros WHERE gid=%s""" % (MunID) )
...
<script>
	...
	var wmsLayer = L.tileLayer.wms('http://localhost:8082/geoserver/sig/wms', {
	    layers: 'sig:sigescolhido', transparent: 'true', format: 'image/png'
	}).addTo(mymap);
...
cursor.execute ("""SELECT ST_Area(ST_Transform(geom,29192)) FROM bairros WHERE gid=%s""" % (MunID))

rows= cursor.fetchall()
for row in rows:
	areamun = (row[0]/1000000)

print 'Área do municipio selecionado: %s km²'% areamun
